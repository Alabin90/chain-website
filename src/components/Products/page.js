// pages/products.js
"use client";
import React, { useRef } from "react";
import ProductCard from "@/components/ProductCard/page";
import Mother from "@/components/Mother/page";


export default function ProductsPage() {
  const scrollRef1 = useRef(null);
  const scrollRef2 = useRef(null);
  const scrollRef3 = useRef(null); // new ref for third row if needed

  const scrollLeft = (ref) => {
    if (ref.current) {
      ref.current.scrollBy({ left: -300, behavior: "smooth" });
    }
  };

  const scrollRight = (ref) => {
    if (ref.current) {
      ref.current.scrollBy({ left: 300, behavior: "smooth" });
    }
  };

  const products = [
  {
   id: 1,
  name: "Classic Diamond Pendant (18k)",
  image: "/image/ped.jpeg",
  slug: "classic-diamond-pendant",
  description: "A timeless diamond pendant crafted in 18k gold for everyday elegance.",
  price: 450000,
  oldPrice: 470000,
  sale: true,
  colors: ["#FFD700", "#D2B48C", "#FFA500"],
},
{
  id: 2,
  name: "Royal Cross Gold Pendant (24k)",
  slug: "royal-cross-gold-pendant",
  image: "/image/pen33.jpg",
  description: "Bold and meaningful, a royal cross pendant designed in pure karat gold.",
  price: 400000,
  sale: false,
  colors: ["#FFD700", "#8B4513", "#D2B48C"],
},
{
  id: 3,
  name: "Heart of Gold Pendant (18k)",
  slug: "heart-of-gold-pendant",
  image: "/image/pe3.jpg",
  description: "Romantic and stylish — a heart-shaped gold pendant that speaks love.",
  price: 380000,
  oldPrice: 410000,
  sale: true,
  colors: ["#FFC0CB", "#FFD700", "#FFA07A"],
},
{
  id: 4,
  name: "Cuban Link Pendant (22k)",
  slug: "cuban-link-pendant",
  image: "/image/pe4.jpg",
  description: "Strong and sleek — a Cuban link pendant made for bold statements.",
  price: 370000,
  sale: false,
  colors: ["#FFD700", "#D2B48C", "#A0522D"],
},
  {
    id: 5,
    name: "Vintage Medallion Pendant  (18k)",
    slug: "vintage-medallion-pendant",
    image: "/image/pe5.jpg",
    description: "Inspired by heritage, this medallion pendant blends tradition and luxury.",
    price: 360000,
    oldPrice: 380000,
    sale: true,
    colors: ["#8B4513", "#FFD700", "#D2B48C"],
  },
  {
    id: 6,
    name: "Golden Sunburst Pendant  (20k)",
    slug: "golden-sunburst-pendant",
    image: "/image/bana.jpg",
    description: "Radiating charm and power — the Sunburst pendant shines with brilliance.",
    price: 390000,
    sale: false,
    colors: ["#FFD700", "#E6E6FA", "#B0E0E6"],
  },
];


 const products2 = [
  {
    id: 1,
    name: "18 Karat Golden Grace Women's Hand Chain",
    slug: "18k-golden-grace-womens-hand-chain",
    image: "/image/hab.jpg",
    description: "Elegant 18K gold hand chain, perfect for daily wear or occasions.",
    price: 450000,
    oldPrice: 480000,
    sale: true,
    colors: ['#FFD700', '#DAA520', '#B8860B'],
  },
  {
    id: 2,
    name: "20 Karat Timeless Unisex Gold Anklet",
    slug: "20k-timeless-unisex-gold-anklet",
    image: "/image/han1.jpg",
    description: "Timeless 20K gold anklet for both men and women.",
    price: 350000,
    sale: false,
    colors: ['#FFD700', '#DAA520', '#B8860B'],
  },
  {
    id: 3,
    name: "22 Karat Opulence Beaded Hand Chain",
    slug: "22k-opulence-beaded-hand-chain",
    image: "/image/han40.jpg",
    description: "Luxurious 22K gold hand chain with beaded accents.",
    price: 400000,
    oldPrice: 420000,
    sale: true,
    colors: ['#FFD700', '#DAA520', '#B8860B'],
  },
  {
    id: 4,
    name: "21 Karat Adjustable Aura Gold Anklet",
    slug: "21k-adjustable-aura-gold-anklet",
    image: "/image/han9.jpg",
    description: "Modern 21K gold anklet with adjustable buckle style.",
    price: 370000,
    sale: false,
    colors: ['#FFD700', '#DAA520', '#B8860B'],
  },
  {
    id: 5,
    name: "19 Karat Vintage Charm Gold Hand Chain",
    slug: "19k-vintage-charm-gold-hand-chain",
    image: "/image/han6.jpg",
    description: "Classic 19K vintage hand chain with elegant design.",
    price: 360000,
    oldPrice: 380000,
    sale: true,
    colors: ['#FFD700', '#DAA520', '#B8860B'],
  },
  {
    id: 6,
    name: "24 Karat Royal Luxury Gold Anklet",
    slug: "24k-royal-luxury-gold-anklet",
    image: "/image/han44.jpg",
    description: "Premium 24K gold anklet crafted for true luxury lovers.",
    price: 390000,
    sale: false,
    colors: ['#FFD700', '#DAA520', '#B8860B'],
  },
  {
    id: 7,
    name: "23 Karat Elegance Gold Hand Chain",
    slug: "23k-elegance-gold-hand-chain",
    image: "/image/hand3.jpg",
    description: "Graceful 23K gold hand chain with refined details.",
    price: 355000,
    oldPrice: 370000,
    sale: true,
    colors: ['#FFD700', '#DAA520', '#B8860B'],
  },
];

 const products3 = [
  {
    id: 6,
    name: "Ethereal Essence Multi-Stone Bracelet (18K)",
    slug: "ethereal-essence-multi-stone",
    image: "/image/ne1.jpg",
    description: "A luxury 18K gold bracelet with multi-stone details, crafted for elegance and daily wear.",
    price: 400000,
    sale: false,
    colors: ["#FFC0CB", "#E6E6FA", "#B0E0E6"],
  },
  {
    id: 4,
    name: "Individually Adjustable Cuban Buckle Bracelet (20K)",
    slug: "cuban-buckle-multilayer",
    image: "/image/ne2.jpg",
    description: "Premium 20K Cuban buckle design, bold and adjustable for perfect wrist comfort.",
    price: 450000,
    sale: false,
    colors: ["#FFC0CB", "#8B4513", "#D2B48C"],
  },
  {
    id: 5,
    name: "Vintage Leather Cord Bracelet (22K)",
    slug: "vintage-flower-leather",
    image: "/image/ne3.jpg",
    description: "22K inspired leather cord bracelet with a unique vintage flower touch for timeless charm.",
    price: 500000,
    sale: true,
    colors: ["#8B4513", "#D2B48C", "#A0522D"],
  },
  {
    id: 7,
    name: "Essence Multi-Stone Bracelet (21K)",
    slug: "essence-multi-stone-bracelet",
    image: "/image/p00.jpg",
    description: "Classic pendant bracelet in 21K gold, designed for those who love refined beauty.",
    price: 400000,
    sale: false,
    colors: ["#FFC0CB", "#E6E6FA", "#B0E0E6"],
  },
  {
    id: 7,
    name: "Multi-Stone Bracelet (24K)",
    slug: "multi-stone-bracelet",
    image: "/image/pe2.jpg",
    description: "Bold 24K multi-stone bracelet, handcrafted for a statement luxury look.",
    price: 450000,
    sale: false,
    colors: ["#FFC0CB", "#E6E6FA", "#B0E0E6"],
  },
  {
    id: 8,
    name: "Stone Bracelet (18K)",
    slug: "stone-bracelet",
    image: "/image/han4.jpg",
    description: "Simple yet elegant 18K gold stone bracelet that blends tradition with style.",
    price: 500000,
    sale: false,
    colors: ["#FFC0CB", "#E6E6FA", "#B0E0E6"],
  },
  {
    id: 9,
    name: "Alphabet Pendant (22K)",
    slug: "alphabet-pendant",
    image: "/image/pen55.jpg",
    description: "Personalized alphabet pendant in 22K gold, perfect for gifting or self-expression.",
    price: 455000,
    sale: false,
    colors: ["#FFC0CB", "#E6E6FA", "#B0E0E6"],
  },
];



  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-10 text-center">
        Explore Our Bracelets
      </h1>

      <Mother/>
      <br/>
      <br/>

      {/* Row 1 */}
      <h2 className="text-2xl font-semibold mb-4">PENDANCES </h2>
      <div className="relative mb-10">
        <button
          onClick={() => scrollLeft(scrollRef1)}
          className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white shadow-md rounded-full p-2 hover:bg-gray-100"
          aria-label="Scroll left"
        >
          ◀
        </button>
        <div
          ref={scrollRef1}
          className="flex space-x-6 overflow-x-auto pb-6 snap-x scrollbar-hide px-2"
        >
          {products.map((product) => (
            <div key={product.id} className="snap-start flex-shrink-0 w-64">
              <ProductCard product={product} />
            </div>
          ))}
        </div>
        <button
          onClick={() => scrollRight(scrollRef1)}
          className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white shadow-md rounded-full p-2 hover:bg-gray-100"
          aria-label="Scroll right"
        >
          ▶
        </button>
      </div>

      {/* Row 2 */}
      <h2 className="text-2xl font-semibold mb-4"> Classic karat chain</h2>
      <div className="relative mb-10">
        <button
          onClick={() => scrollLeft(scrollRef2)}
          className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white shadow-md rounded-full p-2 hover:bg-gray-100"
          aria-label="Scroll left"
        >
          ◀
        </button>
        <div
          ref={scrollRef2}
          className="flex space-x-6 overflow-x-auto pb-6 snap-x scrollbar-hide px-2"
        >
          {products2.map((product) => (
            <div key={product.id} className="snap-start flex-shrink-0 w-64">
              <ProductCard product={product} />
            </div>
          ))}
        </div>
        <button
          onClick={() => scrollRight(scrollRef2)}
          className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white shadow-md rounded-full p-2 hover:bg-gray-100"
          aria-label="Scroll right"
        >
          ▶
        </button>
      </div>

      {/* Row 3 */}
      <h2 className="text-2xl font-semibold mb-4">Mystical & Elegant</h2>
      <div className="relative">
        <button
          onClick={() => scrollLeft(scrollRef3)}
          className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white shadow-md rounded-full p-2 hover:bg-gray-100"
          aria-label="Scroll left"
        >
          ◀
        </button>
        <div
          ref={scrollRef3}
          className="flex space-x-6 overflow-x-auto pb-6 snap-x scrollbar-hide px-2"
        >
          {products3.map((product) => (
            <div key={product.id} className="snap-start flex-shrink-0 w-64">
              <ProductCard product={product} />
            </div>
          ))}
        </div>
        <button
          onClick={() => scrollRight(scrollRef3)}
          className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white shadow-md rounded-full p-2 hover:bg-gray-100"
          aria-label="Scroll right"
        >
          ▶
        </button>
      </div>
    </div>
  );
}
